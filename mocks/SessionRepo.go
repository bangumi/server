...
// Create provides a mock function with given fields: ctx, key, userID, s
func (_m *SessionRepo) Create(ctx context.Context, key string, userID uint32, s session.Session) error {
	ret := _m.Called(ctx, key, userID, s)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, string, uint32, session.Session) error); ok {
		r0 = rf(ctx, key, userID, s)
	} else {
		r0 = ret.Error(0)
	}
...
//  - userID uint32
//  - s session.Session
func (_e *SessionRepo_Expecter) Create(ctx interface{}, key interface{}, userID interface{}, s interface{}) *SessionRepo_Create_Call {
	return &SessionRepo_Create_Call{Call: _e.mock.On("Create", ctx, key, userID, s)}
}

func (_c *SessionRepo_Create_Call) Run(run func(ctx context.Context, key string, userID uint32, s session.Session)) *SessionRepo_Create_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(string), args[2].(uint32), args[3].(session.Session))
	})
	return _c
}
...
func (_m *SessionRepo) Get(ctx context.Context, key string) (session.PersistSession, error) {
	ret := _m.Called(ctx, key)

	var r0 session.PersistSession
	if rf, ok := ret.Get(0).(func(context.Context, string) session.PersistSession); ok {
		r0 = rf(ctx, key)
	} else {
		r0 = ret.Get(0).(session.PersistSession)
	}
...
func (_c *SessionRepo_Get_Call) Return(_a0 session.PersistSession, _a1 error) *SessionRepo_Get_Call {
...
// NewSessionRepo creates a new instance of SessionRepo. It also registers the testing.TB interface on the mock and a cleanup function to assert the mocks expectations.
func NewSessionRepo(t testing.TB) *SessionRepo {
	mock := &SessionRepo{}
	mock.Mock.Test(t)
