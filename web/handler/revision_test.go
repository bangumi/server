...
func TestHandler_ListCharacterRevision_HappyPath(t *testing.T) {
	t.Parallel()
	m := &mocks.RevisionRepo{}
	m.EXPECT().ListCharacterRelated(mock.Anything, uint32(9), 30, 0).Return([]model.Revision{{ID: 227062}}, nil)
	m.EXPECT().CountCharacterRelated(mock.Anything, uint32(9)).Return(1, nil)
	...
	if result, ok := r.Data.([]interface{})[0].(map[string]interface{}); ok {
		if id, ok := result["id"].(float64); ok {
			require.Equal(t, uint32(227062), uint32(id))
		}
	}
}
...
func TestHandler_GetCharacterRevision_HappyPath(t *testing.T) {
	t.Parallel()
	m := &mocks.RevisionRepo{}
	m.EXPECT().GetCharacterRelated(mock.Anything, uint32(158925)).Return(model.Revision{ID: 158925}, nil)

	app := test.GetWebApp(t, test.Mock{RevisionRepo: m})

	req := httptest.NewRequest(http.MethodGet, "/v0/revisions/characters/158925", http.NoBody)
