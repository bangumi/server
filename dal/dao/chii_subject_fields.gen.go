...
type SubjectField struct {
	...
	Year     int32     `gorm:"column:field_year;type:year(4);not null;comment:放送年份"`            // 放送年份
	Mon      int8      `gorm:"column:field_mon;type:tinyint(2);not null;comment:放送月份"`          // 放送月份
	WeekDay  int8      `gorm:"column:field_week_day;type:tinyint(1);not null;comment:放送日(星期X)"` // 放送日(星期X)
	Date     time.Time `gorm:"column:field_date;type:date;not null;comment:放送日期"`               // 放送日期
