...
type Subject struct {
	...
	UID         string       `gorm:"column:subject_uid;type:varchar(20);not null;comment:isbn / imdb"` // isbn / imdb
	...
	Summary     string       `gorm:"column:field_summary;type:mediumtext;not null;comment:summary"`       // summary
	Field5      string       `gorm:"column:field_5;type:mediumtext;not null;comment:author summary"`      // author summary
	Volumes     uint32       `gorm:"column:field_volumes;type:mediumint(8) unsigned;not null;comment:卷数"` // 卷数
	...
	OnHold      uint32       `gorm:"column:subject_on_hold;type:mediumint(8) unsigned;not null;comment:搁置人数"` // 搁置人数
	Dropped     uint32       `gorm:"column:subject_dropped;type:mediumint(8) unsigned;not null;comment:抛弃人数"` // 抛弃人数
