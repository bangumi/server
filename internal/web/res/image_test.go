...
func TestSelectImageByType(t *testing.T) {
	t.Parallel()

	personImage := res.PersonImage("temp")
	img, ok := res.SelectImageByType(personImage, "large")
	...
	img, ok = res.SelectImageByType(subjectImage, "large")
	require.Equal(t, ok, true, "get img url")
	require.Equal(t, img, "https://lain.bgm.tv/pic/cover/l/temp")

	userAvatar := res.Avatar{}.Fill("temp")
	img, ok = res.SelectImageByType(userAvatar, "large")
	require.Equal(t, ok, true, "get img url")
