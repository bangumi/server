...
func (c Command) UpdateEpisodeCollection(
	ctx context.Context,
	userID model.UserID,
	...
	s, err := c.subject.Get(ctx, episode.SubjectID)
	if err != nil {
		if errors.Is(err, domain.ErrNotFound) {
			c.log.Error("unexpected err, missing subject for episode")
	...
	_, err = c.collect.GetSubjectCollection(ctx, userID, s.ID)
	if err != nil {
		if errors.Is(err, domain.ErrNotFound) {
	...
	if err = c.collect.UpdateEpisodeCollection(ctx, userID, s.ID, episodeID, collectionType, time.Now()); err != nil {
